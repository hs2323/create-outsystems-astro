...
prompts.override(yargs(hideBin(process.argv)).parse());
...
function injectIntegrations(targetDir) {
    ...
    JSON.stringify(
      {
        ...
        exports: {
          "./html": "./html/index.js",
          "./html/client": "./html/client.js",
          "./html/server": "./html/server.js",
        },
      },
      null,
      2,
  ...
  // Point the dependency at the local copy
  ...
  console.log("🔌 Injected outsystems integrations");
}
...
function updateAstroConfig(projectDir, selectedFrameworks) {
  ...
  if (!fs.existsSync(configPath)) {
    console.warn(
      "⚠️ astro.config.mjs not found, skipping integration cleanup.",
    );
    return;
  }
  ...
  content = content.replace(/integrations:\s*\[\s*,/g, "integrations: [");
...
function updateMultiAstroPage(projectDir, selectedFrameworks) {
  ...
  if (!fs.existsSync(pagePath)) {
    console.warn(
      `⚠️ ${path.relative(projectDir, pagePath)} not found, skipping page cleanup.`,
    );
    return;
  }
  ...
  const frameworkMap = {
    ...
    svelte: {
      import:
        /import\s+SvelteStore\s+from\s+['"].*?svelte\/Store\.svelte['"];?\s*\n?/g,
  ...
  console.log(
    `✨ Cleaned up components in ${path.relative(projectDir, pagePath)}`,
  );
}
...
function packageInstall(targetDir) {
  try {
    ...
    if (packageManager === "unknown") {
      console.warn(
        "⚠️ Could not detect package manager — keeping all lockfiles.",
      );
    } else {
